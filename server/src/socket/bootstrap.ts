...
import { getRoomByPlayerId, setPlayerConnected } from "../rooms/RoomManager.js";
...
export function createSocketServer(httpServer: HttpServer): Server {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(
      "game:rejoin",
      async (payload: { playerId: string; roomId: string }, callback) => {
        try {
          ...
          socket.to(roomId).emit("room:playerJoined", {
            player: {
              ...
              token:
                room.players.find((p) => p.playerId === playerId)?.token ?? "",
